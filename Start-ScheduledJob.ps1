...
foreach ($job in $jobs) {
    ...
    # If the job hasn't run in the last year or month, start the job
    if ($job.Name -like "*Yearly*" -and $lastRun.EndTime -lt ($today.AddYears(-1))) {
        Start-VBOJob -Job $job -RunAsync | Out-Null
        Write-Host "STARTED | Job:" $job.Name " | Last run:" $lastRun.EndTime
    } elseif ($job.Name -like "*Monthly*" -and $lastRun.EndTime -lt ($today.AddMonths(-1))) {
        Start-VBOJob -Job $job -RunAsync | Out-Null
        Write-Host "STARTED | Job:" $job.Name " | Last run:" $lastRun.EndTime
    } else {
