...
Start-Transcript -Path "C:\Scripts\Logs\VBOJobCleanup\VBOJobCleanup_sched_Job_$timestamp.log"
...
$LogFile = "C:\Scripts\Logs\VBOJobCleanup\VBOJobCleanup-$(Get-Date -Format dd-MM-yyyy_HHmm).log"
$Jobs = $org | Get-VBOJob -Name "*Shared Mailbox"
$MembersUIDlog = "C:\Scripts\Logs\VBOJobCleanup\VBOGroupMemberOutput.log"
######################################################################
...
foreach ($Job in $Jobs) {
    $JobtoRun = Get-VBOJob -Name $Job.Name
    ...
    ForEach-Object { $Warnings += $SeperatorWarnings1 }

    $SeperatorWarnings2 = $JobLog.Title -match "$Seperator2"
    ForEach-Object { $Warnings += $SeperatorWarnings2 }

    $SeperatorWarnings3 = $JobLog.Title -match "$Seperator3"
    ForEach-Object { $Warnings += $SeperatorWarnings3 }
    ...
    foreach ($JobWarning in $Warnings) {
        ...
        elseif ($GroupMemberNames.OfficeID -ccontains $UID) {
            ...
            if ($ExcludedUsersinJob.User.OfficeId -eq $User.OfficeId) {
                ...
                ForEach-Object { $ProcessedUsers += $ExcludeUser.User.OfficeID }
